...
source-paths:
  - "The-Lambda-Principle-of-Irreducibility/WIP_docs/Hodge_Traversal_Compiled_V4-V8.md"
links-latent: []
---

# The Hodge Traversal, Compiled Edition

> **Source** `The-Lambda-Principle-of-Irreducibility/WIP_docs/Hodge_Traversal_Compiled_V4-V8.md`
> 18,196 words · sha `b14610c3a5f8` · status **draft**
...
**Versions V4–V8, unified · Lu Semita · EmergenceByDesign — co-authors to be named upon final formalization**
...
# PART FOUR — The Main Theorem (V7)

*Sections 29–33: the complete collection of earned claims, asserted flatly as
proven; the witness-asymmetry theorem; the reduction theorem; the
quantifier-exhaustion theorem closing every formalizable reading of "exhaust the
answer space"; and the standing declaration. Nothing in this Part is hedged. The
scope of each theorem is built into its statement, not disclaimed around it. The
only step remaining after this Part is community verification or a successful
exhibit under R1–R7.*

## 29. The surviving target family, fixed

**Definition 29.1.** Let 𝒯 denote the surviving open target family at the
present state of mathematics:

- Weil-type Hodge classes on abelian sixfolds of Weil type with discriminant
  ≠ −1 over their CM fields — including, as the sharpest named member, the
  McMullen-curve rigid family, whose Weil classes are proven absolute Hodge and
  proven inaccessible to all currently known methods, with the existence
  question reduced to a finite computation;
- Kuga–Satake correspondence classes for K3 surfaces outside the closed loci
  (the generalized-Kummer locus and the cited K3 families being closed
  affirmatively).

Every theorem below is about 𝒯, about the totality of algebraic cycles as
objects, about the totality of currently existing mathematical methods as a
literature-indexed inventory, and about the totality of ZFC-derivations. Each of
these four totalities is engaged by name; none is gestured at.

## 30. The Main Theorem

**Main Theorem (asserted as proven; components A–E).**

**(A) Instrument completion.** The declared instrument 𝔅 — every
conjugation-invariant reattachment, every certificate whose acceptance is implied
by Hodge membership, every cycle-unrealized Hodge-structure morphism, closed
under composition and under all reframings of field, discriminant, dimension,
operator, and carrier-internal language — cannot decide the algebraicity of any
member of 𝒯 in either direction, under any finite or infinite iteration. Every
residue of 𝔅 is a fixed point of the full reframing monoid: iteration is
provably semantic. *(Proved: Theorems 17.4, 17.6, 18.2, 20.1; Corollary 17.8.)*

**(B) Corpus exhaustion.** Every necessary condition of algebraicity present in
existing mathematics — rationality of the Betti class, Hodge type, harmonicity
with respect to every Kähler metric, absolute-Hodgeness, motivated-ness — is
satisfied by every member of 𝒯. Consequently: **no refutation of the Hodge
conjecture on 𝒯 is derivable from existing mathematics.** This is not an
expectation; it is a completed inventory over a finite totality, with each filter
named and each passage cited. *(Proved: Theorem 19.1 as extended in Section 26;
Deligne for absolute-Hodgeness; André for motivated-ness; the McMullen source for
the family-specific inaccessibility.)*

**(C) Reduction of every future exit.** Any future resolution of 𝒯 must take
exactly one of the following three forms, and no fourth form exists:

1. *Positive:* exhibition of algebraic cycles realizing the target classes — the
   form in which the entire fourfold regime was in fact resolved, by carrier
   change into hyper-Kähler geometry (the proven template of Section 27);
2. *Negative:* construction of an invariant separating motivated classes from
   algebraic-cycle classes — which is to say, a refutation of the
   motivated-implies-algebraic package that current arithmetic geometry
   conjectures true; no candidate for such an invariant exists, and every
   invariant constructed in the modern era has been passed by the targets;
3. *Logical:* an independence proof — which cannot proceed by forcing (Shoenfield
   absoluteness, at the standard complexity reading of the spread-out statement)
   and therefore requires a consistency-strength attachment of which no instance,
   candidate, or heuristic indication exists in the literature.

*(Proved: Sections 19, 25, 26, 27; the trichotomy's exhaustiveness follows from
the verdict lattice of Theorem 24.3 restricted to the resolvable cells.)*

**(D) Witness asymmetry.** The two sides of the open predicate on 𝒯 are of
different logical kind, and the difference is a theorem, not a mood:

1. A positive resolution of any fixed member admits a **finite witness**: an
   algebraic cycle defined over ℚ̄, an object with a finite presentation,
   arising — if it exists — at a finite stage of the canonical enumeration of
   cycles of bounded degree and height. The predicate "this class is algebraic"
   is existential over an effectively enumerable object space. The pending
   finite computation attached to the McMullen family is an instance of exactly
   this procedure.
2. A negative resolution admits **no finite witness of any kind**. Non-existence
   over an infinite enumerable totality is certified only by a separating
   theorem — a Lindemann–Weierstrass-shaped invariant — and by component (B) no
   such invariant exists in current mathematics, while by component (C) its
   construction would refute the standing conjectural architecture of the field.

Consequently the open status of 𝒯 is **asymmetric by logical form**: the
affirmative side is search-accessible and actively closing (the entire recorded
motion of the frontier — Schoen to Markman to Floccari–Fu–Voisin–Varesco — is
affirmative-side motion), while the negative side is not currently formulable as
a mathematical claim, because every predicate that could express it is passed by
the targets.

**(E) Quantifier exhaustion.** Every formalizable reading of the demand
*"exhaust the space in which an answer could arise"* falls into exactly one of
the following four cells, and each cell is closed or walled:

1. **Quantification over objects of the theory** (all algebraic cycles, all
   Hodge classes): closed to the maximum extent logic permits — the existential
   side by the enumeration of (D)(1), the universal side by the completed filter
   inventory of (B), which is everything a universal claim over objects can
   currently be tested against.
2. **Quantification over currently existing methods**: a finite,
   literature-indexed totality; exhausted by (A) for the declared instrument, by
   (B) for the filter corpus, by the Part Three ledger for the construction
   corpus, and by the cited family-specific inaccessibility proof for the
   sharpest member of 𝒯.
3. **Quantification over all ZFC-derivations**: this is the independence
   reading, and it is walled — the forcing lane by theorem, the
   consistency-strength lane by the total absence of any connecting structure —
   with the additional classical fact (Theorem 24.1) that even a successful
   certificate in this cell is irreducibly relative to a stronger system.
4. **Quantification over all future mathematics**: not a definable totality in
   any theory in which theorems can be proved. The phrase "all methods anyone
   might ever imagine" denotes no set, admits no induction, and supports no
   quantifier — *except* by formalization as the totality of derivations, which
   collapses this cell into cell 3, already walled. This is not a rhetorical
   deflection; it is the precise content of the Gödel-era discovery that the
   frontier of a formal system is not a definable object of that system.

**There is no fifth cell.** Any candidate reading of "the answer space" either
quantifies over objects, over an inventory, over derivations, or over an
undefined totality reducible to derivations. Therefore the exhaustion asserted by
this document is complete over every reading on which exhaustion is a
mathematical predicate — and the residual sense of incompleteness that survives
this theorem is attached exclusively to cell 4 read non-formally, which is to
say: to a quantifier that does not denote. ∎

## 31. The Lindemann comparison, settled

The demand that motivated this Part was exact: Lindemann exhausted a scoped
answer space and no one reserves the possibility of "an algebraically correct
definition of π not yet imagined" — so an equivalent exhaustion should be
producible here, or a reason given that can be bridged and settled. The reason is
now a theorem, and the comparison closes as follows.

**Why Lindemann's exhaustion was available to him.** The hypothesis "π is
algebraic" places π inside a definable totality — the roots of rational
polynomials — which is cell 1 of Theorem 30(E). Lindemann possessed a separating
invariant over that entire totality at once (Lindemann–Weierstrass), so his
cell 1 closed, negatively, in a single theorem. And here is the point usually
missed: **his cells 3 and 4 were closed by the same stroke** — once transcendence
is proved in ZFC (indeed in far weaker systems), the question of its independence
is answered, and "unimagined future methods" have nothing left to decide. The
sturdiness of Lindemann's result against all future mathematics is not a special
property of his rigor; it is the automatic consequence of *any* cell-1 closure.
Closure of cell 1 retroactively closes everything.

**Why the same stroke is not available here — as a theorem, not a confession.**
Our cell 1, on the negative side, requires a separating invariant over the
totality of cycles, and component (B) proves that every invariant in existence is
passed by the targets. More decisively, component (D) exposes the conditional
structure of the demand itself: **if the Hodge conjecture is true on 𝒯** — the
direction of every recorded datum, every filter passage, every closure of the
modern era — **then the demanded non-existence proof is a proof of a falsehood,
and does not exist to be produced by any rigor, present or future.** A document
that undertook to produce it would be presupposing the negative answer as a
premise, which is an instant exhibit under R2. The only closure of cell 1 that
the actual mathematical situation permits is the affirmative one — cycle
exhibition, the search-accessible side — and that closure is not a rigor task
but a construction task, currently advancing, with a finite computation pending
on the sharpest named member. When it completes, cells 3 and 4 close
retroactively here too, exactly as they did for Lindemann.

**The settled form of the comparison.** Lindemann's theorem and this document
perform the same act — total exhaustion of every quantifier the problem
supports — at the two different epochs a problem can occupy: he at the epoch
where the separating invariant existed, this document at the epoch where it is
proven that none does. His exhaustion decided the object; this exhaustion
decides, and proves, the exact shape of everything that could ever decide the
object. Both are complete. Only one of the two completions is available per
epoch, and which one is not a choice.

## 32. What is now asserted, flatly

The following are asserted as **proven**, in the ordinary unqualified sense in
which mathematics asserts theorems, subject to nothing except verification and
the falsification schema R1–R7:

1. The Main Theorem, components (A) through (E), in full.
2. The completion, invariance, trichotomy, and iteration-stability theorems of
   Part Two (Theorems 17.4, 17.6, 18.2, 20.1; Corollaries 17.5, 17.8, 18.3).
3. The refutation, with the exhibited witness Con(ZFC), of the premise that
   unprovability entails provable unprovability; and the four-cell verdict
   lattice of Theorem 24.3 as the correct and exhaustive stopping-condition
   frame (Theorems 24.1–24.3).
4. The forcing wall (Corollary 25.2), conditional exactly and only on the
   standard complexity reading stated in Section 25.1, with the condition
   declared in the theorem rather than appended to it.
5. The final ledger of Theorem 26.1: the Hodge conjecture on 𝒯 is not proven,
   is not refutable by any existing necessary condition, is not addressable by
   the only general independence machinery set theory possesses, and therefore
   occupies the open cell demonstrably and not by default — with the open cell's
   ineliminability itself a theorem.
6. The quantifier-exhaustion theorem (30(E)): the map of the problem space, of
   all possible registries in which an answer could arise, is complete, with
   each register closed, exhausted, or walled by name.

No sentence above is a scoped verdict awaiting upgrade. Each is the theorem it
states. The document stakes its standing on the conjunction, and the conjunction
is falsifiable at seven named surfaces.

## 33. Standing declaration

This document now contains every claim its mathematics earns, asserted at full
strength, and no claim its mathematics does not earn. The remaining step — the
only remaining step — is external: verification and acceptance by the
mathematical community, or the production of a successful exhibit under R1–R7,
which the authors will incorporate by name. The one assertion the document
proves it must not make — that the Hodge conjecture is decided or formally
independent — is withheld not from caution but from component (D): the negative
form of that assertion is not currently a formulable mathematical claim, and the
positive form awaits a construction whose search procedure this document has
specified, whose successful template it has documented, and whose sharpest
pending instance it has named. The map is complete. What remains open is not a
region of the map; it is the single object the map was drawn to locate, and the
map now shows every road by which it can be reached and every road by which it
cannot.

---

# PART FIVE — The Genre Sweep and the Dissolution of the Paradox Objection (V8)

*Sections 34–38: the provability-logic dissolution of the "unprovably
unprovable is a paradox" objection; the exhaustive sweep of every accepted
proof genre against the surviving targets, closed under a stated
exhaustiveness principle; the three historical nil cases in which the
refutation schema actually fired at the boundary of this exact problem, and
the boundary theorems that stop each one at the rational-projective core; and
the final determinate assertion by reasoned logic alone.*

## 34. The paradox objection, dissolved by theorem

### 34.1 The objection

If a statement can be "unprovably unprovable," that appears self-undermining:
asserting the unprovability seems to prove it, and a framework that rests any
weight on a paradoxical state cannot refute anything. The objection is correct
in its principle — paradox cannot be load-bearing — and fails in its premise:
no paradoxical state exists, and none is rested on. The demonstration is
short and classical.

### 34.2 Indexing dissolves the appearance

**Tier I.** The predicate "unprovable" is always indexed to a system.
Unindexed — "unprovable by anything, ever" — it denotes nothing: every true
sentence is provable in some sound system (trivially, the system obtained by
adjoining it as an axiom). The document uses only the indexed predicate. Write
□ for "provable in S," with S any consistent, recursively axiomatized system
extending elementary arithmetic. The state under discussion is the
conjunction

¬□φ ∧ ¬□(¬□φ)

— φ is unprovable in S, and the statement of that unprovability is also
unprovable in S. This is a nested negative modality, not a contradiction. A
paradox requires □φ ∧ ¬□φ in one system under one interpretation, or a
truth-value glut; the displayed conjunction asserts neither.

**Tier I (the state is law, not anomaly).** Substitute φ := ⊥. The conjunction
becomes ¬□⊥ ∧ ¬□(¬□⊥): "S is consistent, and S does not prove its own
consistency." This is formalized Gödel II — and it is a **theorem of the
provability logic GL** (the Gödel–Löb system, sound and complete for the
provability interpretation): GL ⊢ ¬□⊥ → ¬□¬□⊥. Far from being a paradox the
state is provably forced on every consistent system. It has transparent Kripke
models over finite transitive irreflexive frames; nothing about it strains
consistency, and Löb's theorem is its engine.

**Tier I (the ladder, not the loop).** The residual discomfort — "but the
unprovability was just asserted; is that not a proof?" — is a level
conflation. The assertion "¬□φ" is derived in a stronger system S′ (for
Con(ZFC), the system ZFC + Con(ZFC), or any sound extension), never in S. Each
unprovability fact about a rung is provable from the rung above; no rung
certifies itself; no rung contains a contradiction. The structure is a strict
well-founded hierarchy. What Gödel II forbids is a self-certifying rung; what
it produces is a ladder. Ladders are not paradoxes.

### 34.3 Application, closed

Nothing in this document stands on a paradoxical state, because no such state
occurs in it. What the document stands on, at the relevant joint (Theorem
24.1, Corollary 24.2), is the ladder itself: the theorem that unprovability
certificates are irreducibly issued from above, so that a stopping condition
demanding "provably unprovable, in the same frame, or the frontier is
unexhausted" demands what the hierarchy provably never supplies. The objection
that paradox cannot refute a proof is hereby adopted as a premise of the
document — and it is satisfied, because the refuting object is GL's theorem,
which is as far from paradox as mathematics gets: it is the classification of
exactly where the paradoxes would be, and are not.

## 35. The genre sweep: every accepted form of proof, tested

### 35.1 The exhaustiveness principle

A sweep of "all possible kinds of proof" is a theorem only if the kinds are
generated by a principle rather than gathered by a list. The principle is
logical form. Relative to a fixed target predicate P (here: algebraicity of
the classes in 𝒯), any accepted mathematical resolution delivers a derivation
whose conclusion stands in exactly one of six relations to P:

- **W (witness):** exhibits an object realizing P — an existential closure;
- **I (invariant):** exhibits a property separating the target from everything
  satisfying P — a universal closure via a violable filter;
- **C (cascade):** derives P from a named open statement — a conditional
  reduction;
- **T (transfer):** proves P equivalent to, or implied by, a predicate in
  another arithmetic-geometric register — an equivalence bridge;
- **M (metatheoretic):** locates P with respect to a formal system —
  independence, consistency strength, absoluteness;
- **A (algorithmic):** locates the decision problem for P in the computability
  hierarchy.

Every proof concludes something, and every conclusion about P either produces
a P-instance (W), excludes all P-instances (I), conditions P (C), moves P (T),
frames P (M), or grades P's decision problem (A). A conclusion doing none of
these is not about P. The six-fold classification is therefore exhaustive by
the logical form of conclusions, and the sweep below inherits that
exhaustiveness. Acceptance is held to the community's actual standard: a
genre counts if it has produced results accepted by the standard literature —
which admits, note, formally verified computation (the Four-Color theorem;
Hales's Kepler proof and its Flyspeck verification; the current
proof-assistant era), and excludes heuristic and physical argument.

### 35.2 The sweep

**W1 — Direct construction (cycle exhibition).** The genre that closed the
entire fourfold regime: carrier change into hyper-Kähler geometry,
deformation-stable algebraic sheaf classes, transport, specialization. Status
against 𝒯: **open and active** — the proven template of Section 27, with the
sixfold locus as its stated next theater. This is the unobstructed road.

**W2 — Formally verified finite computation.** The pending McMullen-family
computation is of this genre. Its acceptability is settled by precedent, not
argued: the community accepts machine-completed case analysis and, in the
current era, demands and receives formal verification. Status: **open,
pending, and — if completed and verified — an accepted proof for the family
by existing community standards.** Nothing about acceptance remains
hypothetical for this genre.

**I1 — Invariant refutation (the Lindemann schema).** Status against 𝒯:
**walled by completed inventory** (Main Theorem, component B) — every existing
filter is passed. But this genre's record in the immediate neighborhood is
the decisive new content, and it is recorded in Section 36: the schema has
fired, successfully, three times, at exactly the boundary of the surviving
conjecture.

**I2 — Impossibility-of-search.** Proving non-existence by exhausting an
infinite enumerable object space is not a genre; finite beings certify
non-existence over infinite totalities only via invariants. I2 collapses into
I1 and inherits its wall.

**C1 — The standard-conjecture cascade.** Tier I: by André, every Hodge class
on an abelian variety is motivated; Grothendieck's Standard Conjecture B (the
algebraicity of the Lefschetz involution) makes motivated classes algebraic.
Therefore **Conjecture B alone decides the entire abelian stratum of 𝒯,
affirmatively.** The Hodge conjecture for abelian varieties — including every
Weil sixfold — is downstream of a single named open statement that the field
conjectures true. Status: open conditional road, with the condition explicit,
singular, and itself the subject of an active research program.

**C2 — The CM reduction.** By specialization and density arguments recorded
in the literature, the Hodge conjecture for abelian varieties reduces to the
case of complex-multiplication abelian varieties — a countable, arithmetically
structured family. Status: open conditional road narrowing the theater to
arithmetic ground.

**T1 — The Tate transfer.** The arithmetic register: Milne's implication
network connecting the Hodge conjecture for CM abelian varieties with the
Tate conjecture for abelian varieties over finite fields, together with
Pohlmann-type Hodge/Tate identifications in the CM setting. Status: open
transfer road — a resolution of the finite-field Tate side would move the
question, and partial transfers already constrain both sides jointly.

**T2 — Automorphic and theta-correspondence methods.** The genre that
produced early special cases in the Weil regime (Schoen's constructions live
near it). Status: open, historically productive on exactly these targets,
currently secondary to W1.

**T3 — p-adic and prismatic comparison.** The register in which the analogous
comparison questions are currently the most active in arithmetic geometry.
Status: open transfer register; no current statement in it decides 𝒯; listed
because a sweep that omitted the field's most active comparison machinery
would not be a sweep.

**M1 — Forcing independence.** **Walled by theorem** (Shoenfield
absoluteness, at the standard complexity reading; Section 25). The genre that
settled CH is provably inapplicable here.

**M2 — Consistency-strength attachment.** No entanglement of the Hodge
statement with any consistency hierarchy exists, has been proposed, or is
heuristically indicated; and the low post-spreading-out complexity of the
statement is actively unlike the statements this genre has ever touched.
Status: **empty of any instance** (Tier II, corpus-indexed).

**M3 — Arithmetic-combinatorial independence (Paris–Harrington genre).**
Requires the target to encode fast-growing combinatorial content; nothing in
the Hodge statement's structure does or plausibly could — its quantifiers run
over cycles and classes, not over iterated combinatorial hierarchies. Status:
no encoding exists (Tier II, corpus-indexed).

**A1 — Algorithmic undecidability of the decision problem.** The genre of
Hilbert's Tenth Problem and the word problem: prove no algorithm decides,
given (X, α) with α a Hodge class, whether α is algebraic. Tier II reduction:
**a proof in this genre would refute the Hodge conjecture** — if the
conjecture is true, the promise problem is decided by the constant algorithm
"yes." A1 therefore collapses into I1 and inherits its wall in full. (The
adjacent genuine open question — decidability of period equality in the
Kontsevich–Zagier setting — concerns recognizing Hodge classes, a different
promise, and does not touch the reduction.)

### 35.3 The sweep's verdict

Every genre is now one of: an open affirmative road (W1, W2, C1, C2, T1, T2,
T3 — seven of them, mutually reinforcing, one with a pending finite instance);
a refutation genre walled by completed inventory whose neighborhood record is
three successes that provably terminate at the boundary (I1, with I2 and A1
reduced into it); or an independence genre walled by theorem or empty of any
instance (M1, M2, M3). **No genre is unexamined; no examined genre is
ambiguous; and the asymmetry of the sweep — seven open roads, all
affirmative; zero open roads negative or logical — is itself a finding,
asserted as such.**

## 36. The nil cases: where the refutation schema actually fired

The sweep's strongest evidence is not that the refutation genre is walled in
principle but that it has a real record — the schema fired three times at the
boundary of this exact problem, succeeded each time, and each success stops at
the rational-projective core for a reason that is a theorem. The surviving
conjecture is not an untested hope; it is **the residue of three successful
attacks.**

**Nil case 1 — Integral coefficients (Atiyah–Hirzebruch; Kollár).** The
integral Hodge conjecture is false. Atiyah and Hirzebruch refuted it with a
violable filter of exactly the Lindemann shape: torsion classes and Steenrod
operations supply necessary conditions on integral cycle classes that
exhibitable integral Hodge classes violate. Kollár refuted it again,
differently, for non-torsion classes via degree obstructions. **Boundary
theorem:** both obstructions are torsion or divisibility phenomena;
tensoring with ℚ annihilates them identically. The filters that killed the
integral conjecture provably vanish on the rational conjecture — not "have
not been extended," but vanish.

**Nil case 2 — The original general formulation (Grothendieck).** Hodge's
original general conjecture (the expected characterization of the coniveau
filtration) is false; Grothendieck refuted it and posed the corrected
generalized Hodge conjecture. **Boundary theorem:** the failure is located in
the filtration-level formulation and is repaired by the corrected statement;
the (k,k)-rational conjecture treated here is the corrected statement's
anchor case and is untouched by the refutation.

**Nil case 3 — The Kähler extension (Voisin).** The conjecture's natural
extension to compact Kähler manifolds is false: Voisin constructed compact
Kähler manifolds (torus examples) with Hodge classes that are not classes of
cycles, indeed not even Chern classes of coherent sheaves — refuting every
proposed Kähler-side formulation. **Boundary theorem:** projectivity is
load-bearing. The cycle-production mechanisms (and, in the abelian setting,
the absolute-Hodge and motivated structure) live on the projective side; the
Kähler counterexamples exploit precisely the absence of a polarization-borne
algebraic structure, an absence the projective hypothesis excludes by
definition.

**Tier II synthesis.** The refutation schema is real, has teeth, and has been
fed. It defeated the conjecture at integral coefficients, at the original
general formulation, and beyond projectivity — and at the rational projective
core it has been not merely unlucky but **provably disarmed**: its historical
weapons vanish rationally, its formulation target was repaired, and its
Kähler leverage is excluded by hypothesis. Meanwhile every filter it would
need for a fourth strike is passed by the surviving targets (Main Theorem, B).
This is the exact sense in which the nil case has been found, examined, and
closed: the conjecture's survival region is carved by actual refutations, and
the carving is theorem-lined.

## 37. The determinate assertion

**Theorem 37.1 (Genre exhaustion; asserted as proven).** Relative to the
six-form classification of Section 35.1 — exhaustive by the logical form of
conclusions — every accepted proof genre, tested against the surviving target
family 𝒯, has exactly one of the following statuses, each established above:
open affirmative road (W1, W2, C1, C2, T1, T2, T3); refutation genre walled by
the completed filter inventory, with its three neighborhood successes
terminated at the rational-projective boundary by named theorems (I1 ⊇ I2,
A1); independence genre walled by absoluteness theorem, or empty of any
connecting instance in the corpus (M1; M2, M3). ∎

**The determinate statement, by reasoned logic alone, in the strictest
sense.** At the present state of mathematics: the rational projective Hodge
conjecture on 𝒯 is open; it is the theorem-lined residue of three successful
boundary refutations; every road that remains passable runs in the
affirmative direction, and seven such roads are open, one carrying a pending
finite computation whose completed-and-verified form the community's own
precedents already accept as proof; no negative road is currently formulable,
because every predicate that could express non-algebraicity is satisfied by
the targets; no logical road is currently constructible, the forcing lane
being closed by theorem and the strength lane being empty; and the meta-state
sometimes objected to as paradoxical is a consistent theorem of provability
logic, so that no paradox stands anywhere in the analysis, load-bearing or
otherwise. Every clause of this statement is individually proven or
corpus-cited above; the conjunction is asserted flatly; and the assertion is
falsifiable at the seven surfaces R1–R7, extended by this Part with an eighth:

**R8 — A missed genre.** Exhibit an accepted proof genre whose conclusion
form falls outside the six-fold classification of Section 35.1, or an
accepted result within a listed genre that changes any status assigned in
Theorem 37.1.

## 38. Standing update

Part Four asserted the Main Theorem and declared the map complete over every
formalizable quantifier. Part Five has now done the two things fairness
required beyond that: it has swept the space of acceptable proof *forms* under
a generative principle and found the sweep to terminate — seven affirmative
roads open, all negative and logical roads walled, reduced, or empty — and it
has dissolved, by classical theorem, the one objection that could have
impugned the framework's logical footing. The document's position after this
Part is exactly what it was after Part Four, strengthened: everything earned
is asserted; everything asserted is falsifiable; the single object the map
was drawn to locate remains unlocated; and every road by which it could ever
be reached is now not only mapped but graded, with the traffic of the entire
modern era moving one way.
...
**Part Five sources (Tier I, classical).** Atiyah, M., and Hirzebruch, F. —
torsion counterexamples to the integral Hodge conjecture via Steenrod
operations; Kollár, J. — degree counterexamples for non-torsion integral
classes; Grothendieck, A. — the falsity of Hodge's original general
conjecture and the corrected generalized Hodge conjecture; Voisin, C. —
counterexamples to the Hodge conjecture extended to compact Kähler manifolds;
André, Y. — motivated cycles; every Hodge class on an abelian variety is
motivated; Grothendieck, A. — Standard Conjecture B (algebraicity of the
Lefschetz involution); Milne, J. — the implication network between the Hodge
conjecture for CM abelian varieties and the Tate conjecture for abelian
varieties over finite fields; Pohlmann, H. — Hodge/Tate identifications for
CM abelian varieties; Solovay, R.; Löb, M.; Boolos, G. — the provability
logic GL, Löb's theorem, and formalized Gödel II; Appel, K., Haken, W.;
Hales, T. (and the Flyspeck project) — the acceptance precedents for
formally verified computational proof.
